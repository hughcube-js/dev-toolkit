...
class UniappManifestUpdater {
    ...
    parseArgs() {
        ...
        // 如果没有任何参数，显示帮助
        if (!this.version && !this.h5PublicPath && args.length === 0) {
            this.showHelp()
            process.exit(0)
        }

        // 如果没有任何有效参数但有其他参数，提示错误
        if (!this.version && !this.h5PublicPath && args.length > 0 && !this.debugMode) {
            console.error('❌ 请至少提供一个有效参数 (--version 或 --h5-public-path)')
...
功能:
  ...
  3. 自动计算 versionCode (每个版本部分占用3位数字)
  4. 如果不存在 h5 配置，会自动创建
  5. 支持相对路径和完整 URL (publicPath 必须以 / 结尾)
  6. 使用 JSON5 安全解析带注释的 JSON 文件
  7. 支持跨平台运行 (Windows, macOS, Linux)
`)
    }
