...
`)
    ...
    isPagePathMatched(pagePath, filter) {
        ...
        // 直接路径匹配（包含匹配）
        return pagePath.includes(filter)
    }
    ...
    generateCompileModeConfig() {
        try {
            ...
            if (this.dumpPagesFilter) {
                console.log(`   过滤器: ${this.dumpPagesFilter}`)
                console.log(`   原始页面数: ${pagesConfig.pages.length}，过滤后: ${filteredPages.length}`)
            }
