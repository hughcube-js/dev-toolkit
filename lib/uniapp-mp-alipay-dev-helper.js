...
配置文件:
  生成位置: <dist-dir>/.mini-ide/project-ide.json
  核心配置 (默认启用):
    ...
    watchDistDir() {
        ...
        try {
            ...
            let watcher = fs.watch(this.configDir, {recursive: true}, (eventType, filename) => {
                if (eventType === 'rename' && filename && filename.includes('project-ide.json')) {
                    setTimeout(() => {
                        if (fs.existsSync(this.distDir) && !fs.existsSync(this.configFile)) {
                            console.log('🔍 检测到配置文件被删除:', filename)
                            this.generateConfig()
                        }
                    }, 500)
                }
