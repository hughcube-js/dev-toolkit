...
class MpAlipayUploader {
    ...
    parseArgs() {
        ...
        this.appId = getArgValue(['--app-id', '-a']) || process.env.ALIMP_APP_ID || null
        this.distDir = getArgValue(['--dist-dir', '-d']) || process.env.ALIMP_DIST_DIR || null
        this.version = getArgValue(['--version', '-v']) || process.env.ALIMP_VERSION || null
        this.minidevConfig = getArgValue(['--config', '-c']) || process.env.ALIMP_MINIDEV_CONFIG_FILE || null
        this.versionDescribe = getArgValue(['--version-describe']) || process.env.ALIMP_VERSION_DESCRIBE || null
...
环境变量 (优先级低于 CLI 参数):
  ALIMP_APP_ID              小程序 APPID
  ALIMP_DIST_DIR            构建产物目录
  ALIMP_VERSION             版本号
  ALIMP_MINIDEV_CONFIG_FILE minidev 配置文件路径
  ALIMP_VERSION_DESCRIBE    版本描述
...
`)
    ...
    validateConfig() {
        ...
        if (!this.appId) {
            ...
            errors.push('   请使用 --app-id 参数或设置环境变量 ALIMP_APP_ID')
        }

        if (!this.distDir) {
            errors.push('❌ 缺少构建产物目录')
            errors.push('   请使用 --dist-dir 参数或设置环境变量 ALIMP_DIST_DIR')
        ...
        if (!this.version) {
            errors.push('❌ 缺少版本号')
            errors.push('   请使用 --version 参数或设置环境变量 ALIMP_VERSION')
        } else {
            // 验证版本号格式（去掉v前缀后验证）
        ...
        if (!this.minidevConfig) {
            ...
            errors.push('   请使用 --config 参数或设置环境变量 ALIMP_MINIDEV_CONFIG_FILE')
